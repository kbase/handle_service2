#!/usr/bin/env bash
#
# Script to run the python handle_service_container_test.py locally or on GitHub Actions.
# Builds and mounts handle service and mongo docker containers, and then calls
# the python script.
#
...
PYTHONPATH="$current_dir":$PYTHONPATH python -m pytest container_test/handle_service_container_test.py
exit_code=$?
